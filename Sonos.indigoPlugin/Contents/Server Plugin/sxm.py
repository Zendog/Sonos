...
# These are now defined in this same file, no import needed
# from sxm.models import QualitySize, RegionChoice, XMChannel, XMLiveChannel
...
class QualitySize(Enum):
    SMALL_32k = "SMALL_32k"
    MEDIUM_128k = "MEDIUM_128k"
    LARGE_256k = "LARGE_256k"


class XMChannel:
    def __init__(self, id: str, name: str, channel_number: str, guid: str,
                 short_description: str = "", long_description: str = "",
                 category_name: str = "", is_favorite: bool = False):
        self.id = id
        self.name = name
    ...
    def from_dict(data: Dict[str, Any]):
        return XMChannel(
            ...
            guid=data.get("guid") or data.get("assetGuid", ""),
...
class XMLiveChannel:
    ...
    @staticmethod
    def from_dict(data: Dict[str, Any]):
        return XMLiveChannel(data)

    def set_stream_quality(self, quality: QualitySize):
        pass

    def set_hls_roots(self, primary: str, secondary: str):
        self.primary_hls = type("HLS", (), {"url": primary})
        self.secondary_hls = type("HLS", (), {"url": secondary})
...
class SXMClientAsync:
    ...
    async def get_channels(self) -> List[dict]:
        """Fetches full channel metadata using SiriusXM v4 API with extended metadata."""
        postdata = {
            "consumeRequests": [],
            "resultTemplate": "web",
            "alerts": [],
        ...
        try:
            modules = data["moduleList"]["modules"]
            for module in modules:
                if (
                    "moduleResponse" in module
                    and "contentData" in module["moduleResponse"]
                    and "channelListing" in module["moduleResponse"]["contentData"]
                ):
                    return module["moduleResponse"]["contentData"]["channelListing"]["channels"]
        except Exception:
            self._log.error("Failed to extract channelListing data from response.")
            self._log.error(traceback.format_exc())
