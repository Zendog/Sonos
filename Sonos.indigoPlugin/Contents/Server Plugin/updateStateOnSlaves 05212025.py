    ...
    def updateStateOnSlaves(self, dev):
        ...
        try:
            ...
            # 🔄 NEW: Propagate playback metadata to slaves
            playback_keys = [
            ...
            for slave_dev in slave_devices:
                for key in playback_keys:
                    ...
                    try:
                        self.logger.warning(f"⚠️ it is me 1")                        
                        slave_dev.updateStateOnServer(key, value)
                    except Exception as e:
                        self.logger.warning(f"⚠️ Failed to update '{key}' on {slave_dev.name}: {e}")

            # Update devices that are now standalone
            for indigo_device in indigo.devices.iter("self"):
